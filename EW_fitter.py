...
--linelistfile LINELISTFILE; LINELISFILE=only filename of the line-list-to-be-fit file, use this option if the name is not of the format labframe.shortlinelist_
...
--fitcont ; boolean option, if present tries to fit continuum on the fly using jrr.spec.fit_autocont, default false
...
parser.add_argument("--frac_overlap")
...
if args.dx is not None:
    ...
    dx = 300.
if args.frac_overlap is not None:
    frac_overlap = float(args.frac_overlap)
else:
    frac_overlap = 0.2
...
if args.stackbyneb:
    ...
    listname = 'emission'
    fout = path + 'byneb_stack_fitted_emission_measured.txt'
if args.stackbystars:
    labels = [
        'magestack_bystars_standard', \
        ]
    listname = 'photospheric'
    fout = path + 'bystars_stack_fitted_photospheric_measured.txt'
...
if args.path is not None:
    ...
    path = HOME + 'Dropbox/MagE_atlas/Contrib/EWs/'+listname+'/'  # directory to store the resulting output files
subprocess.call(['mkdir -p ' + path], shell=True)  # create output directory if it doesn't exist
if args.allesi:
    labels = [
        's1723_center_a_esi', \
        's1723_center_b_esi', \
...
if args.fout is not None:
    ...
    whichspec = labels[0] if len(labels) == 1 else 'multiple'
    fout = path + whichspec + '_fitted_'+listname+'_measured.txt'
if not args.keepprev:
    plt.close('all')
if args.linelistpath is not None:
    linelistpath = args.linelistpath
else:
    linelistpath = HOME + 'Dropbox/MagE_atlas/Contrib/EWs/linelists/' # directory where lines-to-be-fit files are kept
...
for ii in range(0, len(specs)):
    try:
        ...
        zz_dic = {'EMISSION': specs['z_neb'][ii], 'FINESTR': specs['z_neb'][ii],
                  'PHOTOSPHERE': specs['z_stars'][ii] if specs['fl_st'][ii] == 0 else specs['z_neb'][ii],
                  'ISM': specs['z_ISM'][ii], 'WIND': specs['z_ISM'][ii]}
        zz_err_dic = {'EMISSION': specs['sig_neb'][ii] if specs['fl_neb'][ii] == 0 else specs['sig_ISM'][ii],
                      'FINESTR': specs['sig_neb'][ii] if specs['fl_neb'][ii] == 0 else specs['sig_ISM'][ii],
                      'PHOTOSPHERE': specs['sig_st'][ii] if specs['fl_st'][ii] == 0 else specs['sig_neb'][ii],
                      'ISM': specs['sig_ISM'][ii], 'WIND': specs['sig_ISM'][ii]}
        ...
        if 'esi' in shortlabel:
            ...
            sp_orig = m.open_esi_spectrum(specdir + filename,
                                          getclean=True)  # open_esi_spectrum() can open any other spectra as well, if the other spectra has been converted to desired format
        elif 'stack' in shortlabel:
            (sp_orig, LL_dummy) = jrr.mage.open_stacked_spectrum(mage_mode, which_stack='standard', colfnu='fweightavg',
                                                                 colfnuu='fweightavg_u')  # alt_infile= Put the filename of the stacked spectrum file here
            resoln = 3e5 / 200.  # vel resol of 200km/s from file /Users/acharyya/Dropbox/mage_atlas/Contrib/S99/stack-A-sb99-fit.txt
            dresoln = 40.  #
        else:
            (sp_orig, resoln, dresoln, LL_dummy, zz_sys) = jrr.mage.wrap_open_spectrum(shortlabel, mage_mode, addS99=False,
                                                                                       zchoice='neb', MWdr=True)

        # -----------fitting continuum unless asked to use existing continuum column in spectrum dataframe----------------------------
        if args.fitcont:  # fitting continuum on the fly, before fitting lines, unless already auto-fitted continuum by jrr.mage while reading in spectra
            m.fit_autocont(sp_orig, zz_sys, line_path, filename)
            if 'flam_autocont' not in sp_orig: sp_orig['flam_autocont'] = jrr.spec.fnu2flam(sp_orig['wave'],
                                                                                            sp_orig.fnu_autocont)  # added this because now I use the new jrr.spec.fit_autocont() which, unlike jrr.mage.fit_autocont(), does not create flam_autocont and rest_***_cont columns
            (dummy, rest_fnu_autocont, dummy) = jrr.spec.convert2restframe(sp_orig['wave'], sp_orig.fnu_autocont,
                                                                           sp_orig.fnu_autocont, zz_sys, 'fnu')
            sp_orig['rest_fnu_autocont'] = pd.Series(rest_fnu_autocont)
            (dummy, rest_flam, dummy) = jrr.spec.convert2restframe(sp_orig['wave'], sp_orig.flam_autocont, sp_orig.flam_u,
                                                                   zz_sys, 'flam')
            sp_orig['rest_flam_autocont'] = pd.Series(rest_flam)
        elif args.usefnucont is not None:  # choosing a fnu_cont column
            if colfnucont in sp_orig:
                sp_orig.drop('fnu_autocont', 1,
                             inplace=True)  # removing any pre-existing fnu_autocont column, as now we're going to create a new fnu_autocont column
                ...
                sys.exit()
        elif args.useflamcont is not None:  # choosing a flam_cont column
            if colflamcont in sp_orig:
                if 'flam_autocont' in sp_orig: sp_orig.drop('flam_autocont', 1,
                                                            inplace=True)  # removing any pre-existing flam_autocont column, as now we're going to create a new flam_autocont column
        ...
        line_full = line_full[line_full['wave'].between(sp_orig.wave.values[0], sp_orig.wave.values[-1])].reset_index(
            drop=True)  # only so many lines from the linelist are supposed to be present given the wavelength coverage
        # ------------Preparing to plot----------------------------------------
        ...
        if display_only_success and args.extract is None:
            for jj in range(len(n_arr)):
                xmin = xstart + n_arr[jj] * dx - dx * frac_overlap
        ...
        # ------------------------------------------------------------
        n_subplot = nrow * ncol
        count = 0  # to count total number of line fitted
        n_subarr = np.split(n_arr, np.arange(n_subplot, n_subplot * len(n_arr) / n_subplot + 1,
                                             n_subplot))  # np.array_split(n_arr, int(np.ceil(len(n_arr)/float(nrow*ncol))))
        ...
        for ss in range(len(n_subarr)):
            ...
            if not args.noplot:
                ...
                if not args.see and not args.extract: plt.title(
                    shortlabel + "  z=" + str(zz_sys) + '.\n Vertical lines legend: Blue=initial guess of center,' + \
            ...
            for fc, jj in enumerate(n_arr):
                if args.extract is not None:
                    ...
                    xmin = xstart + jj * dx - dx * frac_overlap  # dx*frac_overlap for overlap between frames, so as to not miss any lines
                    xmax = min(xmin + dx, xlast)
                if not args.noplot:
                    if args.extract is None:
                        ax1 = fig.add_subplot(n, 1, fc + 1)
                    else:
                        ax1 = fig.add_subplot(nrow_actual, ncol_actual, fc + 1)
                sp = sp_orig[sp_orig['wave'].between(xmin, xmax)]
                if not args.plotfnu:
                    ymin = np.min(
                        sp.flam_u) * 0.98  # setting ylimits for plotting, to little lower than minimum value of the error
                    ymax = np.max(sp.flam) * 1.5  # little higher than maximum flux value
                else:
                    ymin = np.min(
                        sp.fnu_u) * 0.98  # setting ylimits for plotting, to little lower than minimum value of the error
                ...
                if not args.noplot:
                    try:
                        ...
                        break
                    if not args.extract and not args.saveeps: plt.text(xmin + dx * 0.005, ax1.get_ylim()[1] * 0.9,
                                                                       'Frame ' + str(int(jj) + 1))
                if not args.fullmad and not args.nofit:
                    m.fit_some_EWs(line, sp, resoln, shortlabel, line_table, dresoln, sp_orig,
                                   args=args)  # calling line fitter
                if not args.noplot:
                    ax2 = ax1.twiny()
            ...
            if not args.noplot:
                ...
                if not args.plotfnu:
                    fig.text(0.02, 0.5, r'$f_\lambda$ (ergs/s/cm$^2$/' + AA + ')', va='center', rotation='vertical')
                else:
...
if any(lab in shortlabel for lab in
       ['rcs0327-E', 's1723']) and not args.nodered and args.see is None and args.extract is None:
    ...
    line_table['f_redcor'], line_table['f_redcor_u'] = m.extinct(line_table.rest_wave, line_table.f_line,
                                                                 line_table.f_line_u, E, E_u, doMC=False, size=niter)
    line_table['f_Suplim_redcor'], dummy = m.extinct(line_table.rest_wave, line_table.f_Suplim,
                                                     np.zeros(len(line_table)), E, E_u, doMC=False, size=niter)
    line_table['f_redcor_SNR'] = np.abs(line_table['f_redcor']) / line_table['f_redcor_u']
...
if shortlabel == 'rcs0327-E' and not args.nodered and args.see is None and args.extract is None:
    short_table = line_table[
        ['line_lab', 'rest_wave', 'EWr_fit', 'EWr_fit_u', 'EWr_Suplim', 'EW_signi', 'f_line', 'f_line_u', 'f_SNR',
         'f_Suplim', 'f_signi', 'f_redcor', 'f_redcor_u', 'f_redcor_SNR', 'f_Suplim_redcor']]
    ...
    if 'MMT' in shortlabel:
        suffix = '.mmt'
    elif 'ESI' in shortlabel:
        suffix = '.esi'
