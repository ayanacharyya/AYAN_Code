...
for ii in range(0,len(labels)):
    ...
    if not args.noweight: vel_offset = np.multiply(vel_offset, short_table[weightby])/np.sum(short_table[weightby]) #weighting by EWr_fit
    ...
    bin_centers = np.pad(bin_centers,pad_width=1, mode='constant',constant_values=(bin_centers[0]-np.diff(bin_edges)[0], bin_centers[-1]+np.diff(bin_edges)[0])) #just to amke histogram look nicer
