...
def detect(table, line, ew_thresh=4, fSNR_thresh=1):
    if table[table['line_lab'].eq(line)].EW_signi.values[0] > ew_thresh and (
            table[table['line_lab'].eq(line)].f_line.values[0] / table[table['line_lab'].eq(line)].f_line_u.values[
        0] > fSNR_thresh):
        return True
    else:
        return False
    ...
    for line in lines_num:
        try:
            if detect(table, line):
                ...
                new_tab = new_tab.append(
                    table[table['line_lab'].eq(line)][['label', 'line_lab', 'EWr_fit', 'EWr_fit_u', 'EWr_Suplim', \
                                                       'EW_signi', 'f_line', 'f_line_u', 'f_Suplim', 'f_signi']],
                    ignore_index=True)  #
